...
const mockQuestions = [{ question: 'Q1: How much is 2+2?', answer: '4' }];
...
describe('Getting answer for question', () => {
  ...
  it('Should send prompt with question and answer when question do not have predefined answer', async () => {
    ...
    sendPromptToGPTMock.mockImplementationOnce(
      () => new Promise((res, rej) => res('Answer'))
    );

    render(
      <MakeAnswerForm
        questionsWithAnswers={[
          { question: 'Q1: How much is 2+2?', answer: undefined },
        ]}
      />
    );
  ...
  it('SHould send prompt with question, user answer and correc answer when question has answer', async () => {
    ...
    render(
      <MakeAnswerForm
        questionsWithAnswers={[
          { question: 'Q1: How much is 2+2?', answer: '123' },
        ]}
      />
    );
    ...
    const input = screen.getByTitle('answer');
    const sendButton = screen.getByText('Send answer');
    const finalPrompt = prepareGPTPromptWithCorrectAnswer(
      'Q1: How much is 2+2?',
      'my answer',
      '123'
    );
